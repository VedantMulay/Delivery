...
public class DeliveryManager {
    ...
    public void reload() {
        plugin.getLogger().info("Reloading deliveries data...");
        deliveries.clear();
        try {
            load();
            plugin.getLogger().info("Deliveries reloaded successfully.");
        } catch (Exception e) {
            plugin.getLogger().severe("An error occurred while reloading deliveries: " + e.getMessage());
            e.printStackTrace();
        }
    }
    ...
    public void claimDelivery(Player player, String deliveryId) {
        ...
        if (player.hasPermission(delivery.getPermission())) {
            if (canClaim(player, deliveryId)) {
                ...
                // Additional logic for claiming the delivery
            } else {
